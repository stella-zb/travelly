...
export const List = ({timeslots, deleteAttraction}: PropTypes) => {
  const categorizeTimeslot = (slot: {id:number, attraction_id:number, name:string, photo:string, travel_mode:string, start_time:number, end_time:number, first_name:string|null, last_name:string|null}) => {
    ...
    if (slot.attraction_id === null && slot.travel_mode == 'WALKING') {
      ...
      return <Timeslot key={slot.id}><IconDiv>{getIcon('attraction')}<Copy>{moment.unix(slot.start_time).utc().format('hh:mm a')}</Copy></IconDiv><ContentDiv><Attraction key={slot.id} id={slot.attraction_id} name={slot.name} img={slot.photo} editable={false} deleteAttraction={deleteAttraction} firstName={slot.first_name} lastName={slot.last_name} /></ContentDiv></Timeslot>
    }
  };
