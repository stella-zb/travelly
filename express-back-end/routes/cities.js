...
module.exports = db => {
    ...
    db.query(
      ...
      axios
        ...
        .then(results => {
          ...
          return Promise.all([
            axios.get(
              `https://api.foursquare.com/v2/venues/explore?near=${city}?&limit=25&client_id=${FOURSQUARE_KEY}&client_secret=${FOURSQUARE_SECRET}&v=20200120`
            ),
            axios.get(
  ...
  router.post(`/:itinerariesId/`, (req, res) => {
    ...
    db.query(
      ...
      FULL OUTER JOIN timeslots on attractions.id = timeslots.attraction_id
      WHERE name = $1;
      `,
      [name]
    ).then(query => {
